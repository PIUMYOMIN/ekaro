...
const SCard = ({ label, value, sub, accent }) => {
  const accents = {
    emerald: "border-l-4 border-emerald-400 bg-emerald-50",
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500">Trend view:</span>
            {GROUP_BY.map(g => (
          ...
          <button onClick={handleFetch}
            className="ml-auto px-4 py-1.5 bg-gray-900 text-white text-xs font-semibold rounded-lg hover:bg-gray-800">
            Load Report
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="text-xs text-gray-400 font-medium">
            Report period: <span className="text-gray-700 font-semibold">{s.from} → {s.to}</span>
          </p>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Gross Merchandise Value</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Commission</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Platform Delivery Fees</p>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Platform Revenue (Pyonea Earns)</p>
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
              ...
              <div className="flex gap-2 flex-wrap">
                <input
                  ...
                  className="border border-gray-300 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-green-500 w-52"
                />
              ...
              <table className="min-w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredOrders.length === 0 ? (
                    <tr><td colSpan={15} className="px-5 py-10 text-center text-gray-400">No orders found.</td></tr>
                  ) : filteredOrders.map(o => (
                    <React.Fragment key={o.order_id}>
                      <tr className="hover:bg-gray-50 transition-colors">
                        <td className="px-3 py-3 font-mono font-semibold text-gray-900">{o.order_number}</td>
                        <td className="px-3 py-3 whitespace-nowrap text-gray-500">{o.order_date?.slice(0,10)}</td>
                        <td className="px-3 py-3">
                          <p className="font-medium text-gray-900">{o.buyer_name}</p>
                          <p className="text-gray-400 text-[10px]">{o.buyer_email}</p>
                        </td>
                        <td className="px-3 py-3">
                          <p className="font-medium text-gray-900">{o.seller_name}</p>
                        </td>
                        <td className="px-3 py-3 max-w-32">
                          <p className="truncate text-gray-600">{o.items_summary}</p>
                          <p className="text-gray-400 text-[10px]">{o.items_count} item{o.items_count !== 1 ? "s" : ""}</p>
                        </td>
                        <td className="px-3 py-3 text-right font-medium text-gray-900">{fmtMMK(o.subtotal)}</td>
                        <td className="px-3 py-3 text-right text-gray-500">{fmtMMK(o.shipping_fee)}</td>
                        <td className="px-3 py-3 text-right text-gray-500">{fmtMMK(o.tax_amount)}</td>
                        <td className="px-3 py-3 text-right font-bold text-gray-900">{fmtMMK(o.total_amount)}</td>
                        <td className="px-3 py-3 text-right">
                          <p className="font-medium text-teal-700">{fmtMMK(o.commission_amount)}</p>
                          <p className="text-gray-400 text-[10px]">{(o.commission_rate * 100).toFixed(1)}%</p>
                        </td>
                        <td className="px-3 py-3">
                        ...
                        <td className="px-3 py-3">
                          <button
                            ...
                            className="text-green-600 hover:text-green-800 text-[10px] font-medium whitespace-nowrap"
                          >
                      ...
                      {expandedOrder === o.order_id && (
                        <tr className="bg-green-50">
                          <td colSpan={15} className="px-6 py-3">
                            <p className="text-xs font-semibold text-gray-600 mb-2">Product Items</p>
...
{o.items.map((item, i) => (
                                  <tr key={item.id || item.product_id || item.sku || `item-${o.order_id}-${i}`} className="border-t border-green-100">
                                    ...
                                    <td className="py-1 text-right font-medium text-gray-900">{fmtMMK(item.subtotal)}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                            <div className="mt-2 flex gap-4 text-xs text-gray-500">
                              <span>Payment: <strong className="text-gray-700">{o.payment_method?.replace(/_/g," ")}</strong></span>
                              <span>Escrow: <strong className="text-gray-700">{o.escrow_status}</strong></span>
                              <span>Delivered: <strong className="text-gray-700">{o.delivered_at?.slice(0,10) ?? "—"}</strong></span>
                            </div>
                          </td>
                        </tr>
                      )}
