  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-gray-900">{initial ? 'Edit Campaign' : 'New Campaign'}</h3>
        <button onClick={onCancel} className="text-gray-400 hover:text-gray-600">✕</button>
      </div>
      {error && <p className="text-sm text-red-600 flex items-center gap-1"><ExclamationCircleIcon className="h-4 w-4"/>{error}</p>}

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">Campaign Name (internal)</label>
      ...
      {/* Audience */}
      <div>
        <label className="text-xs font-medium text-gray-600 block mb-1">Audience</label>
        <select value={form.audience} onChange={e => set('audience', e.target.value)}
          className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:ring-2 focus:ring-green-500 focus:outline-none">
        ...
        {form.audience === 'buyers_by_city' && (
          <input value={form.audience_filter?.city || ''} onChange={e => setFilter('city', e.target.value)}
            ...
            className="mt-2 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 focus:outline-none"/>
        )}
        {form.audience === 'sellers_by_tier' && (
          <select value={form.audience_filter?.tier || 'gold'} onChange={e => setFilter('tier', e.target.value)}
            className="mt-2 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-green-500 focus:outline-none">
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="text-xs font-medium text-gray-600">Email Content (HTML supported)</label>
        ...
        {preview ? (
          <div className="border border-gray-200 rounded-xl min-h-[200px] max-h-[400px] overflow-y-auto p-4 text-sm text-gray-700 leading-relaxed"
          ...
          <textarea value={form.body_html} onChange={e => set('body_html', e.target.value)}
            ...
            rows={10}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-mono focus:ring-2 focus:ring-green-500 focus:outline-none resize-none"/>
        )}
        <p className="text-xs text-gray-400 mt-1">Tip: Use simple HTML. Add links with &lt;a href="..."&gt;. An unsubscribe link is added automatically for newsletter subscribers.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            <span className="font-semibold text-green-700">{subscribers.total.toLocaleString()}</span> active subscribers
            {subscribers.unconfirmed > 0 && <span className="ml-2 text-amber-600">({subscribers.unconfirmed} pending confirmation)</span>}
          </p>
        </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {campaigns.map(c => (
                    <tr key={c.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3 text-xs text-gray-600">{AUDIENCE_LABELS[c.audience] || c.audience}</td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3 text-right text-gray-600 hidden sm:table-cell">
                        ...
                        {c.delivered_count > 0 && <span className="text-green-600 ml-1">({c.delivered_count} delivered)</span>}
                      </td>
                      <td className="px-5 py-3 text-right text-xs text-gray-400 hidden md:table-cell">{fmtDate(c.sent_at)}</td>
                      <td className="px-5 py-3">
      ...
      {view === 'subscribers' && (
        <div className="space-y-4">
          <input value={subSearch} onChange={e => setSubSearch(e.target.value)}
            ...
            className="w-full sm:w-72 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:outline-none"/>
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            {subList.length === 0 ? (
              <p className="text-center py-10 text-sm text-gray-400">No subscribers found.</p>
            ) : (
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 text-xs font-medium text-gray-500 uppercase tracking-wide">
                  <tr>
                ...
                <tbody className="divide-y divide-gray-50">
                  {subList.map(s => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3 text-gray-600 hidden sm:table-cell">{s.name || '—'}</td>
                      <td className="px-5 py-3 text-xs text-gray-400 hidden md:table-cell capitalize">{s.source}</td>
                      <td className="px-5 py-3 text-xs text-gray-400">{fmtDate(s.confirmed_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <p className="text-xs text-gray-400">{subMeta.total?.toLocaleString() || 0} active subscribers total</p>
        </div>
      )}
    </div>
  );
