...
const Stars = ({ rating, count }) => {
  ...
  return (
    <div className="flex items-center gap-1">
      <div className="flex">
        {[1, 2, 3, 4, 5].map((i) => (
          <StarIcon key={i} className={`h-3 w-3 ${i <= filled ? "text-amber-400" : "text-gray-200"}`} />
        ))}
      </div>
      {(rating > 0 || count > 0) && (
...
// ── Swipe-up delivery zone ticker — same pattern as CategoryCard ──────────────
...
// ── Delivery zone strip (always shown at card bottom) ─────────────────────────
const DeliveryZoneStrip = ({ sellerId, sellerProfile }) => {
  const [zones, setZones] = useState([]);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    // Try to derive zones from seller profile data already on the product
    if (sellerProfile?.delivery_areas_summary) {
      const s = sellerProfile.delivery_areas_summary;
      setZones(s.length ? s : ["Whole Myanmar"]);
      setLoaded(true);
      return;
    }
...
const ProductCard = ({ product, className = "" }) => {
  ...
  const toggleWishlist = async (e) => {
    ...
    if (!user) { navigate("/login", { state: { returnTo: window.location.pathname } }); return; }
    if (wishLoading) return;
    setWishLoading(true);
    try {
      if (isInWishlist) { await removeFromWishlist(productId); flash("Removed from wishlist"); }
      else              { await addToWishlist(productId);      flash("Added to wishlist ♡"); }
    } catch { flash("Could not update wishlist", "error"); }
  ...
  const handleAddToCart = async () => {
    ...
    setCartLoading(true);
    try { await addToCart(productId, 1); flash("Added to cart ✓"); }
    catch { flash("Could not add to cart", "error"); }
    finally { setCartLoading(false); }
  };

  const sellerName = product.seller?.seller_profile?.store_name
    || product.seller?.store_name
    || product.seller?.name;

  const sellerProfile = product.seller?.seller_profile || product.seller_profile;
    ...
    >
      ...
      <div className="relative flex-shrink-0 overflow-hidden bg-gray-50 aspect-square">
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1">
          {discountPct > 0 && !isOutOfStock && !isUnavailable && (
            <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full leading-tight">-{discountPct}%</span>
          )}
          {product.is_new && !isOutOfStock && !isUnavailable && (
            <span className="bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full leading-tight">NEW</span>
          )}
          {isOutOfStock && (
            <span className="bg-gray-700/80 text-white text-[10px] font-bold px-2 py-0.5 rounded-full leading-tight backdrop-blur-sm">Sold Out</span>
          )}
          {isUnavailable && !isOutOfStock && (
            <span className="bg-gray-400 text-white text-[10px] font-bold px-2 py-0.5 rounded-full leading-tight">N/A</span>
          )}
        </div>
        ...
        {isBuyer && (
          <button onClick={toggleWishlist} disabled={wishLoading}
            className="absolute top-2.5 right-2.5 w-7 h-7 rounded-full bg-white/90 backdrop-blur-sm shadow
                       flex items-center justify-center hover:bg-white hover:scale-110 active:scale-95
                       transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-green-400 disabled:opacity-60"
        ...
        {(product.category?.name_en || product.category?.name_mm) && (
          <div className="absolute bottom-2 left-2">
            <span className="text-[10px] font-medium text-gray-500 bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-full">
      ...
      <div className="flex flex-col flex-grow px-3 pt-3 pb-3 sm:px-4 sm:pt-3.5">
        <Link to={`/products/${slug}`} className="block mb-1.5">
          <h3 className="text-sm font-semibold text-gray-900 line-clamp-2 leading-snug group-hover:text-green-700 transition-colors duration-150">
        ...
        {sellerName && <p className="mt-1 text-[10px] text-gray-400 truncate">by {sellerName}</p>}
              ...
              {discountPct > 0 ? (
                <>
                  <span className="text-base font-bold text-red-600 leading-none">
                    {formatMMK(product.discount_price || product.price * (1 - discountPct / 100))}
                  </span>
                  <span className="text-xs text-gray-400 line-through leading-none">{formatMMK(product.price)}</span>
                </>
              ) : (
                <span className="text-base font-bold text-green-700 leading-none">{formatMMK(product.price)}</span>
              )}
            </div>
            {product.moq > 1 && (
              <span className="text-[10px] text-gray-400 bg-gray-50 border border-gray-200 px-1.5 py-0.5 rounded-md font-medium whitespace-nowrap">
                MOQ {product.moq}
              </span>
            )}
          </div>
        ...
        {isBuyer && (
          <button onClick={handleAddToCart}
            ...
            className={`mt-2.5 w-full rounded-xl py-2 text-xs font-semibold flex items-center justify-center gap-1.5
            ...
            {!isUnavailable && !isOutOfStock && <ShoppingCartIcon className="h-3.5 w-3.5 flex-shrink-0" />}
            {cartLabel}
          </button>
        )}
      </div>

      {/* ── Delivery zone strip (swipe-up ticker) ── */}
      {sellerId && (
        <DeliveryZoneStrip sellerId={sellerId} sellerProfile={sellerProfile} />
      )}
    </motion.div>
  );
};
